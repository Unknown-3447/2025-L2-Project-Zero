...
def string_check(question, valid_answers, num_letters=1):
    ...
    while True:
        ...
        for valid_item in valid_answers:
            if response == valid_item or response == valid_item[:num_letters]:
                return valid_item
...
def get_expenses(exp_type, how_many=1):
    ...
    expense_string = tabulate(expense_frame, headers='keys', tablefmt='psql',
                              showindex=False) if exp_type == "variable" else tabulate(expense_frame[['Item', 'Cost']],
...
5. Choose how to round ticket prices (e.g., to the nearest $1 or 50c) for simple pricing.
6. For each ticket, enter:
   - The buyer's name
   - Their age (12 to 120 years)
   - Payment method (cash or credit; credit has a 5% extra fee)
7. Type 'xxx' when done selling tickets or when you reach your ticket limit.
...
# Calculate and Store Ticket Prices
ticket_prices = {
    "child": round(recommended_price * 2 / 3, 2),  # Children (<16) get 2/3 of adult price
    "adult": recommended_price,  # Adults (16–64) use recommended price
    "senior": round(recommended_price * 1 / 2, 2)  # Seniors (65+) get 1/2 of adult price
}

# Display Expense Summary and Ticket Prices
print()
print(make_statement("Event Cost Summary", "💰"))
print(f"{'Total Expenses':<20}: {currency(total_expenses)}")
print(f"{'Profit Goal':<20}: {currency(target_profit)}")
print(f"{'Ticket Prices':<20}:")
print(f"{'  Child (<16)':<20}: {currency(ticket_prices['child'])}")
print(f"{'  Adult (16–64)':<20}: {currency(ticket_prices['adult'])}")
print(f"{'  Senior (65+)':<20}: {currency(ticket_prices['senior'])}")
print("These prices will be used for ticket sales to meet your profit goal.")

# Ticket Sales Loop
print()
while tickets_sold < quantity:
    print(f"\n--- Ticket {tickets_sold + 1} of {quantity} ---")
    name = not_blank("Name: ")
    if name.lower() == "xxx":
        break
    ...
    if age < 12:
        ...
        continue
    ticket_price = ticket_prices["child"] if age < 16 else ticket_prices["senior"] if age >= 65 else ticket_prices[
        "adult"]
    ...
    tickets_sold += 1
    print(f"{name} has bought a ticket ({pay_method}) for {currency(ticket_price + surcharge)}")

# Summary
ticket_frame = pandas.DataFrame(ticket_dict)
ticket_frame['Total'] = ticket_frame['Ticket Price'] + ticket_frame['Surcharge']
ticket_frame['Profit'] = ticket_frame['Ticket Price'] - (total_expenses / quantity)
...
to_write = [
    ...
    f"Recommended Ticket Price (Adult): ${recommended_price:.2f}",
    f"Child Ticket Price (<16): ${ticket_prices['child']:.2f}",
    f"Senior Ticket Price (65+): ${ticket_prices['senior']:.2f}",
    make_statement("Ticket Sales", "-"),
    ticket_frame.to_string(index=False),
    f"\nTotal Paid: ${total_paid:.2f}",
    f"Total Profit: ${total_profit:.2f}",
    make_statement("Raffle Winner", "-"),
    f"Lucky Winner: {winner}. Their ticket worth ${ticket_won:.2f} is free!" if winner != "No entries" else
    "No raffle due to no ticket sales.",
    f"Adjusted Total Paid: ${total_paid - ticket_won:.2f}",
    f"Adjusted Total Profit: ${total_profit - profit_won:.2f}",
    make_statement(f"Sold {tickets_sold}/{quantity} tickets", "-") if tickets_sold < quantity else make_statement(
        f"All {quantity} tickets sold!", "-")
]
